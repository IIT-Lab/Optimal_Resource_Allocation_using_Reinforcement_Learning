...
class TD3:
    def __init__(self, lr, state_dim, action_dim, max_action):
        ...
        self.actor = Actor(state_dim, action_dim, max_action).to(device)#moving the model to gpu if gpu exists
        self.actor_target = Actor(state_dim, action_dim, max_action).to(device)
        #copy the parameters from the orogonal actor to this target actor to make these identical since they are initialized with random weights
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=lr)#only the actor here since the target actor will be periodically updated with the actor's parameters
    ...
    def clip_action_custom(self, action):
        print("IMplement clip action")
    
    def update(self, replay_buffer, n_iter, batch_size, gamma, polyak, policy_noise, noise_clip, policy_delay):
        
        for i in range(n_iter):
            # Sample a batch of transitions from replay buffer:
            state, action_, reward, next_state, done = replay_buffer.sample_a_transition_from_replay_buffer(batch_size)
